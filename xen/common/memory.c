...
#define RTXEN_CC_SELECT_0     (0)
#define RTXEN_CC_SELECT_8     (8)
static void populate_physmap(struct memop_args *a)
{
    ...
    for ( i = a->nr_done; i < a->nr_extents; i++ )
    {
        ...
        else
        {
            ...
            if ( (d->domain_id >= 3 && (d->domain_id % 2 == 0)) && 
                 (mfn & RTXEN_CC_SELECT_MASK) != RTXEN_CC_SELECT_0 )
            {
                --i;
                printk("%s mfn=%lx & 0x18 != 0, not alloc to dom %d\n", __FUNCTION__, mfn, d->domain_id);
                continue;
            }
            if ( (d->domain_id >= 3 && (d->domain_id % 2 == 1)) && 
                 (mfn & RTXEN_CC_SELECT_MASK) != RTXEN_CC_SELECT_8 )
            {
                --i;
                printk("%s mfn=%lx & 0x18 != 0x08, not alloc to dom %d\n", __FUNCTION__, mfn, d->domain_id);
                continue;
            }
